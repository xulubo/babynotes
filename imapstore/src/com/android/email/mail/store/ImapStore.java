...
public class ImapStore extends Store {
    ...
    public class ImapFolder extends Folder {
        ...
        public Message[] getMessagesSince(Date date, MessageRetrievalListener listener)
                throws MessagingException {
            if (date == null) {
                throw new MessagingException(
                        "date is NULL");
            ...
            String sinceDate = format.format(date);
            ...
            try {
                ArrayList<String> uids = new ArrayList<String>();
                List<ImapResponse> responses = mConnection
                        .executeSimpleCommand(String.format("UID SEARCH SINCE %s NOT DELETED", sinceDate));
                for (ImapResponse response : responses) {
                    if (response.get(0).equals("SEARCH")) {
                        for (int i = 1, count = response.size(); i < count; i++) {
                            uids.add(response.getString(i));
